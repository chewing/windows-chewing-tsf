...
use windows_core::{
    BOOL, BSTR, GUID, IUnknown, Interface, PWSTR, Ref, Result,
    implement
};
...
pub(super) struct LangBarButton {
    ...
    menu: HMENU,
    command_id: u32,
    command_cb: Box<dyn Fn(u32, CommandType)>,
...
impl ITfLangBarItemButton_Impl for LangBarButton_Impl {
    fn OnClick(&self, click: TfLBIClick, _pt: &POINT, _prcareaa: *const RECT) -> Result<()> {
        ...
        (self.command_cb)(self.command_id, cmd_type);
        Ok(())
    }
    ...
    fn OnMenuSelect(&self, wid: u32) -> Result<()> {
        (self.command_cb)(wid, CommandType::Menu);
        Ok(())
    }
